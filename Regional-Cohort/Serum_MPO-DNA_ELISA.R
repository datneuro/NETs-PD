# =============================================================================
# Serum_MPO-DNA_ELISA.R
# Serum MPO-DNA complex ELISA, HC vs PD + severity gradient (Figure 3a)
# LM + emmeans + HC3 robust SE + stratified bootstrap + Bayesian (brms).
# Usage:  Rscript Serum_MPO-DNA_ELISA.R [DATA_FILE] [OUT_DIR] [OUTCOME_VAR] [RUN_BRMS]
# =============================================================================
...
OUTCOME_VAR <- if (length(args) >= 3) args[3] else "lg_MPO_DNA"
...
LOG_FILE <- file.path(OUT_DIR, "E_serum_log.md")
log_md <- function(...) cat(paste0(..., "\n"), file = LOG_FILE, append = TRUE)
cat("", file = LOG_FILE)
log_md("# Serum ELISA Analysis Log - Serum_MPO-DNA_ELISA.R")
...
write_csv(group_table, file.path(OUT_DIR, "data", "E_lm_HC_vs_PD_coefficients.csv"))
write_csv(emm_group_df, file.path(OUT_DIR, "data", "E_lm_HC_vs_PD_emmeans.csv"))
write_csv(boot_group, file.path(OUT_DIR, "data", "E_lm_HC_vs_PD_bootstrap.csv"))
...
write_csv(broom::tidy(fit_sev), file.path(OUT_DIR, "data", "E_lm_severity_coefficients.csv"))
write_csv(emm_sev_df, file.path(OUT_DIR, "data", "E_lm_severity_emmeans.csv"))
write_csv(poly_trend_df, file.path(OUT_DIR, "data", "E_lm_severity_polytrend.csv"))
...
write_csv(corr_results, file.path(OUT_DIR, "data", "E_correlation_PD_clinical.csv"))
...
if (RUN_BRMS) {
  ...
  if (!is.null(fit_group_brm)) {
    saveRDS(fit_group_brm, file.path(OUT_DIR, "data", "E_brms_HC_vs_PD.rds"))
    ...
    write_csv(fixed_tbl, file.path(OUT_DIR, "data", "E_brms_HC_vs_PD_fixed.csv"))
  ...
  for (v in clinical_vars) {
    ...
    if (!is.null(fit_b)) {
      saveRDS(fit_b, file.path(OUT_DIR, "data", paste0("E_brms_PD_", v, ".rds")))
  ...
  if (length(bayes_clinical) > 0) {
    ...
    write_csv(bayes_clinical_df, file.path(OUT_DIR, "data", "E_brms_PD_clinical_summary.csv"))
...
fig_group <- ggplot(serum %>% filter(!is.na(diag), !is.na(outcome)),
  ...
  geom_violin(alpha = 0.15, trim = FALSE, linewidth = 0.2) +
  ...
  labs(
    title = "Serum MPO-DNA by diagnosis",
...
fig_sev <- ggplot(serum %>% filter(!is.na(subDX), !is.na(outcome)),
  ...
  labs(
    title = "Serum MPO-DNA by severity",
    subtitle = paste0("Outcome: ", OUTCOME_VAR),
    x = "Severity",
    y = OUTCOME_VAR
  ) +
...
ggsave(file.path(OUT_DIR, "pdf", "E_serum_group_panels.pdf"), fig_combined, width = 12, height = 5)
ggsave(file.path(OUT_DIR, "png", "E_serum_group_panels.png"), fig_combined, width = 12, height = 5, dpi = 300)
...
if (length(key_clinical) > 0) {
  ...
  if (length(corr_figs) >= 1) {
    ...
    ggsave(file.path(OUT_DIR, "pdf", "E_serum_corr_key.pdf"), p_corr, width = 10, height = 4)
    ggsave(file.path(OUT_DIR, "png", "E_serum_corr_key.png"), p_corr, width = 10, height = 4, dpi = 300)
  }
}
...
message("Done - Serum_MPO-DNA_ELISA.R")
