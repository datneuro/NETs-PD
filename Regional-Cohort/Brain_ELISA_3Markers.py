#!/usr/bin/env python3
"""Brain NET-complex ELISA, 3 NET-specific markers + composite.
...
+ Bayesian-bootstrap posterior P(MSA>PD>HC)."""
...
log(
    df[["id", "dx", "mpo_dna", "ne_dna", "cith3_dna", "composite"]]
    .round(4)
...
def bayes_bootstrap_order(g):
    ...
    B = 4000
    hc, pd, ms = g["HC"], g["PD"], g["MSA"]

    def draw(x):
        w = np.random.dirichlet(np.ones(len(x)), B)
        return (w * x).sum(1)

    mh, mp, mm = draw(hc), draw(pd), draw(ms)
    return (
        float(np.mean((mm > mp) & (mp > mh))),
        float(np.mean(mp > mh)),
        float(np.mean(mm > mh)),
    )
...
for col, name in markers.items():
    ...
    log(f"## {name}")
    g = {
        lv: df.loc[df.dx == lv, col].astype(float).values for lv in ["HC", "PD", "MSA"]
    }
    desc = (
        df.groupby("dx", observed=True)[col].agg(["mean", "std", "min", "max"]).round(4)
    )
    log(desc.to_string())
    H, pkw = stats.kruskal(*g.values())
    F, pan = stats.f_oneway(*g.values())
    ...
    for (c, d, t, p), h in zip(rws, hp):
        ...
        diffs = np.array(
            [
                np.random.choice(g[c.split("-")[0]], 3, True).mean()
                - np.random.choice(g[c.split("-")[1]], 3, True).mean()
                for _ in range(5000)
            ]
        )
        lo, hi = np.percentile(diffs, [2.5, 97.5])
        log(
            f"  {c:7s} diff={d:+.4f} Welch t={t:+.2f} p={p:.4f} p_Holm={h:.4f} | boot95%CI[{lo:+.3f},{hi:+.3f}]"
        )
    p_ord, p_pd, p_msa = bayes_bootstrap_order(g)
    log(
        f"  Bayesian bootstrap: P(MSA>PD>HC)={p_ord:.3f} | P(PD>HC)={p_pd:.3f} | P(MSA>HC)={p_msa:.3f}"
    )
    summary.append(
        {
            "marker": name,
            "HC": round(g["HC"].mean(), 3),
            "PD": round(g["PD"].mean(), 3),
            "MSA": round(g["MSA"].mean(), 3),
            "KW_p": round(pkw, 4),
            "P_ordered": round(p_ord, 3),
            "P_PDgtHC": round(p_pd, 3),
            "P_MSAgtHC": round(p_msa, 3),
        }
    )
...
fig.suptitle(
    "Brain NET-complex ELISA (n=3/group) — individual markers + composite",
    fontsize=11,
    y=1.04,
)
plt.tight_layout()
